...
void MyChunkyList::remove(int index){
    ...
    /*else if(index==0){//remove at head/index 0
         start->items()[0] = "";
         //counter--;
         start->decrement();
         counter--;
         if(start->count() == 0){
             if(start==end){
                 ...
                 start->next()->setprev(nullptr);
                 MyChunkyNode* temp = start;
                 start = start->next();
                 delete temp;
                 return;
        ...
        if(start->items()[index+1] != ""){//shift left
             for(int i = index; i<start->count();i++){
                 start->items()[i] = start->items()[i+1];
                 start->items()[i+1] = "";
             }
            start->countItems();
         }
         if(start!=end){
            if((start->count()+start->next()->count())<=(size/2)){//merge head and adjacent
                if(start->next()->next() == nullptr){//if only two nodes including head
                    MyChunkyNode* newNode = new MyChunkyNode(nullptr, nullptr,size);
                    for(int i = 0; i<size;i++){
                        if(start->items()[i]!=""){
                            newNode->items()[i] = start->items()[i];
                        }
                    }
                    newNode->countItems();
                    int j = 0;
                    for(int i = newNode->count(); i<size; i++){
                        ...
                        if(start->next()->items()[j]!=""){
                            newNode->items()[i] = start->next()->items()[j];
                            j++;
                        }
                    ...
                    MyChunkyNode* newNode = new MyChunkyNode(nullptr, start->next()->next(),size);
                     for(int i = 0; i<size;i++){
                        if(start->items()[i]!=""){
                            newNode->items()[i] = start->items()[i];
                        }
    ...
    else if(index == counter-1){//remove at tail
        end->items()[counter-1] = "";
        end->decrement();
        counter--;
        if(end->count() == 0){//tail becomes empty
            if(end == start){
                delete end;
                start = nullptr;
                end = nullptr;
                ...
                MyChunkyNode* temp = end;
                end->prev()->setnext(nullptr);
                end = end->prev();
                delete temp;
                return;
            }
        ...
        if(end!=start){
            if((end->prev()->count()+end->count())<=(size/2)){
                if(end->prev()->prev() == nullptr){
                    MyChunkyNode* newNode = new MyChunkyNode(nullptr,nullptr,size);
                    for(int i = 0; i<size;i++){
                        if(end->prev()->items()[i]!=""){
                            newNode->items()[i] = end->prev()->items()[i];
                        }
                    }
                    newNode->countItems();
                    int j = 0;
                    ...
                    MyChunkyNode* newNode = new MyChunkyNode(nullptr,end->prev()->prev(),size);
                    for(int i = 0; i<size;i++){
                        if(end->prev()->items()[i]!=""){
                            newNode->items()[i] = end->prev()->items()[i];
                        }
